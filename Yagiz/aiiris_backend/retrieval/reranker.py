...
def rerank(query: str, documents: dict[str], with_score: bool = True , model_name: str = "cross-encoder/ms-marco-MiniLM-L-6-v2", top_n: int = 3) -> List[str]:
    if not documents:
        return []
    ...
    try : 
        ...
        if with_score:
            ...
            return [{"content": doc["content"], "metadata": doc.get("metadata", {})} for doc, _ in top_docs]
