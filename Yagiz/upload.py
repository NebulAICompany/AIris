...
class UploadPipeline:
    ...
    def z(self):
        ...
        for page_num in range(num_pages):
            ...
            # Paragraphs
            page_paragraphs = [p for p in result.paragraphs if p.bounding_regions and p.bounding_regions[0].page_number == page_num]
            ...
            # Tables
            page_tables = [t for t in result.tables if t.bounding_regions and t.bounding_regions[0].page_number == page_num]
        ...
        other_paragraphs = [p for p in result.paragraphs if not p.bounding_regions]
        for paragraph in other_paragraphs:
            sentences = nltk.sent_tokenize(paragraph.content)
            for sentence in sentences:
                metadata = {
                    "content": sentence,
                    "page_number": None,  # sayfa bilgisi yok
                    "type": "text",
                    "source_file": self.source_file
                }
                results.append(metadata)
