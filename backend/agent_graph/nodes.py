...
llm_fast = ChatOpenAI(model=FAST_MODEL, temperature=MODEL_TEMPERATURE, callbacks=[ConsoleCallbackHandler()])
...
class PlanApprovalCheck(BaseModel):
    ...
    wants_changes: bool = Field(
        description="True if the user wants to modify the plan"
    )
    feedback: str = Field(
...
async def human_approval_node(
    ...
    if approval_status == "pending":
        return Command(
            goto=END,
            update={"human_approval_status": "awaiting_feedback"}
        )
    
    # We have user feedback - use LLM to analyze if they approved
    checker = llm_fast.with_structured_output(PlanApprovalCheck)
...
"""
    ...
    return Command(
        ...
        update={
            ...
            "messages": [AIMessage(content="I've proposed a plan above. Would you like me to proceed with this plan, or would you like to make any changes?")]
        }
    )
...
async def orchestrator_node(
    ...
    llm_with_tools = llm_reasoning.bind_tools([WriteTodos], tool_choice="auto")
...
async def document_sub_agent_node(input_data: SubAgentInput) -> Dict[str, Any]:
    ...
    Args:
        input_data: SubAgentInput containing the document_id (used as research topic).
        
    Returns:
        State update with the Summary added to global_context.
    """
    from langchain_core.messages import ToolMessage
    from .tool_definitions import search_specific_document
    
    doc_id = input_data["document_id"]
    
    model_with_tools = llm_fast.bind_tools([search_specific_document])
    ...
    messages = [
        ...
        HumanMessage(content=f"Research Topic: {doc_id}")
    ]
    ...
    if ai_msg.tool_calls:
        for tool_call in ai_msg.tool_calls:
            if tool_call["name"] == "search_specific_document":
                tool_output = await search_specific_document.ainvoke(tool_call["args"])
    ...
    extractor = llm_fast.with_structured_output(Summary)
...
async def synthesis_node(
    ...
    if global_context:
        findings_str = "\n\n".join([
            f"**Document {s.document_id}** (Relevance: {s.relevance_score:.2f}):\n{s.findings}" 
    ...
    response = await llm_reasoning.ainvoke([HumanMessage(content=prompt_content)])
