...
def route_from_start(state: AgentState) -> Literal["clarify_intent_node", "human_approval_node"]:
    """
...
def route_orchestrator(state: AgentState) -> Union[List[Send], Literal["synthesis_node"]]:
    """
    ...
    if selected_docs and not context:
        return [
            Send("document_sub_agent_node", {"document_id": doc_id}) 
...
def build_graph() -> StateGraph:
    ...
    # Conditional entry point: resume at approval if awaiting feedback
