"""
Financial News Database Management
Handles persistent storage and retrieval of aggregated financial news.
"""
...
class NewsDatabase:
    ...
    def find_existing_news_by_links(self, new_links: List[str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for news in existing_news:
                    ...
                    # Check for any match
                    if any(new_link in existing_links for new_link in normalized_new_links):
                        logger.info(f"🔗 Found existing news match: {news['unified_title']}")
                        return dict(news)
                return None
        ...
        try:
                ...
                if article.get('source') and article.get('link'):
                    ...
                    if article.get('published'):
                        try:
                            ...
                            if earliest_published is None or pub_date < earliest_published:
            ...
            with sqlite3.connect(self.db_path) as conn:
                if existing_news:
                    ...
                    existing_earliest = datetime.fromisoformat(existing_news['earliest_published_at']) if existing_news['earliest_published_at'] else None
                    if earliest_published and (not existing_earliest or earliest_published < existing_earliest):
                        new_earliest = earliest_published
                    else:
                        new_earliest = existing_earliest
                    
                    # Update latest published date if needed
                    existing_latest = datetime.fromisoformat(existing_news['latest_published_at']) if existing_news['latest_published_at'] else None
                    if latest_published and (not existing_latest or latest_published > existing_latest):
                    ...
                    logger.info(f"📰 Updated existing news: {unified_title} (ID: {existing_id})")
                    return existing_id
                    ...
                    """, (
                        ...
                        current_time.isoformat(),
                        current_time.isoformat(),
                        earliest_published.isoformat() if earliest_published else None,
