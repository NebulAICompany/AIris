...
@tool
def extract_data_from_text(text: str, data_format: str = "auto"):
    """
    ...
    try:
        ...
        if data_format == "json":
            ...
            return json.dumps(
                {"success": False, "error": f"Unknown data format: {data_format}"}
            )
...
@tool
def convert_to_plottable_format(data: str, format_type: str = "auto"):
    """
    ...
    try:
        ...
        if isinstance(result, pd.DataFrame):
            return json.dumps(
                {"success": True, "data": result.to_dict("list"), "format": "dataframe"}
            )
        elif isinstance(result, np.ndarray):
            return json.dumps(
                {"success": True, "data": result.tolist(), "format": "array"}
            )
        else:
...
VALIDATION_RULES = {
    "pie": {
        "checks": [
            ...
            (
                lambda r, c: r > 100,
                "Pie chart not suitable for more than 100 categories",
            ),
            (
                lambda r, c: c == 1 and r > 50,
                "Pie chart with single column not recommended for more than 50 items",
            ),
        ],
        "negative_check": True,
    },
    "scatter": {
        "checks": [
            (
                lambda r, c: c < 2 and r < 2,
                "Scatter plot requires at least 2 data points or 2 columns",
            ),
    ...
    "bar": {
        "checks": [
            ...
            (
                lambda r, c: c > 30,
                "Bar plot not recommended for more than 30 grouped bars",
            ),
        ]
    },
    "histogram": {
        "checks": [
            (
                lambda r, c: c > 10,
                "Histogram not recommended for more than 10 distributions",
            ),
    ...
    "heatmap": {
        "checks": [
            ...
            (
                lambda r, c: c > 100 or r > 100,
                "Heatmap not recommended for matrices larger than 100x100",
            ),
        ]
    },
    "area": {
    ...
    "heatmap_1d": {
        "checks": [
            ...
            (
                lambda r, c: max(r, c) > 500,
                "1D heatmap not suitable for more than 500 values",
            ),
        ]
    },
    "violin": {
        "checks": [
            (
                lambda r, c: c > 15,
                "Violin plot not recommended for more than 15 groups",
            ),
            (
                lambda r, c: r < 10,
                "Violin plot requires at least 10 data points per group",
            ),
        ]
    },
    "bubble": {
    ...
    "waterfall": {
        "checks": [
            ...
            (
                lambda r, c: r > 50,
                "Waterfall chart not recommended for more than 50 steps",
            ),
    ...
    "radar": {
        "checks": [
            ...
            (
                lambda r, c: c > 20,
                "Radar chart not recommended for more than 20 dimensions",
            ),
            (
                lambda r, c: r > 10,
                "Radar chart not recommended for more than 10 series",
            ),
        ]
    },
    "funnel": {
        "checks": [
            ...
            (
                lambda r, c: r > 15,
                "Funnel chart not recommended for more than 15 stages",
            ),
    ...
    "candlestick": {
        "checks": [
            (
                lambda r, c: c != 4,
                "Candlestick chart requires exactly 4 columns (O,H,L,C)",
            ),
            (lambda r, c: r < 2, "Candlestick chart requires at least 2 time periods"),
            (
                lambda r, c: r > 1000,
                "Candlestick chart not recommended for more than 1000 periods",
            ),
        ],
        "ohlc_check": True,
    },
    ...
    "scatter_3d": {
        "checks": [
            ...
            (
                lambda r, c: r > 10000,
                "3D scatter not recommended for more than 10000 points",
            ),
        ]
    },
}
...
def validate_data_for_plot(
    data: np.ndarray, plot_type: str, config: Dict[str, Any]
) -> tuple[bool, str]:
    ...
    if data.ndim != 2:
        return (
            False,
            f"Data must be 2-dimensional after reshaping, got {data.ndim} dimensions",
        )
    ...
    if rules.get("negative_check"):
        try:
            ...
            if np.any(data < 0):
                return (
                    False,
                    f"{plot_type.capitalize()} chart cannot handle negative values",
                )
    ...
    try:
        ...
        if arr.size == 0:
            return json.dumps(
                {
                    "success": False,
                    "error": "Data is empty, cannot determine suitable plot types",
                }
            )
...
def _prepare_data(
    data: Union[List, np.ndarray, pd.DataFrame, Dict], config: Dict
) -> np.ndarray:
...
def _setup_defaults(data: np.ndarray, config: Dict) -> None:
    ...
    if not config.get("column_names"):
        config["column_names"] = (
            ["Value"] if cols == 1 else [f"Series {i+1}" for i in range(cols)]
        )
    ...
    if not x_vals:
        ...
        config["x_values"] = [str(x) for x in x_vals] + [
            str(i) for i in range(len(x_vals), rows)
        ]


def _add_multi_trace(
    fig: go.Figure, data: np.ndarray, config: Dict, trace_type: Callable, **kwargs
) -> None:
    """Helper to add multiple traces to a figure."""
    for i in range(data.shape[1]):
        fig.add_trace(
            trace_type(y=data[:, i], name=config["column_names"][i], **kwargs)
        )
...
@tool
def create_html_plot(data: str, plot_type: str = "line", config: str = None):
    """
    ...
    try:
        ...
        if isinstance(data, str):
            try:
                ...
                return json.dumps(
                    {
                        "success": False,
                        "error": "Data must be valid JSON string from convert_to_plottable_format",
                    }
                )
        ...
        if data.size == 0:
            return json.dumps(
                {"success": False, "error": "Data is empty after preparation"}
            )
        ...
        if not plot_func:
            ...
            return json.dumps(
                {"success": False, "error": f"Plot type '{plot_type}' is not supported"}
            )
...
def create_histogram_plot(data: np.ndarray, config: Dict) -> go.Figure:
    ...
    for i in range(data.shape[1]):
        fig.add_trace(
            go.Histogram(x=data[:, i], name=config["column_names"][i], opacity=0.7)
        )
...
def create_heatmap_1d_plot(data: np.ndarray, config: Dict) -> go.Figure:
    ...
    x_vals = (
        config["x_values"][: len(values[0])]
        if len(values[0]) <= len(config["x_values"])
        else list(range(len(values[0])))
    )

    fig = go.Figure(
        data=go.Heatmap(
            z=values, colorscale="Viridis", showscale=True, y=["Value"], x=x_vals
        )
    )
...
def create_violin_plot(data: np.ndarray, config: Dict) -> go.Figure:
    ...
    fig = go.Figure()
    _add_multi_trace(
        fig, data, config, go.Violin, box_visible=True, meanline_visible=True
    )
    fig.update_yaxes(title_text=config["y_label"])
    return fig
...
def create_radar_plot(data: np.ndarray, config: Dict) -> go.Figure:
    ...
    for i in range(data.shape[0]):
        name = (
            str(config["x_values"][i])
            if i < len(config["x_values"])
            else f"Series {i+1}"
        )
        fig.add_trace(
            go.Scatterpolar(
                r=data[i, :], theta=config["column_names"], fill="toself", name=name
            )
        )
