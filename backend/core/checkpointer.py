...
async def clear_thread_checkpoints(thread_id: str) -> bool:
    """
    Remove persisted checkpoint rows for a single LangGraph thread/session.

    This is used as a recovery path when a previous interrupted tool call leaves
    Anthropic tool_use blocks without matching tool_result blocks in checkpointed state.
    """
    global _db_connection

    if not _db_connection or not thread_id:
        return False
    ...
    try:
        ...
        for table_name in table_names:
            ...
            async with _db_connection.execute(pragma_query) as cursor:
                async for row in cursor:
            ...
            delete_query = f'DELETE FROM "{table_name}" WHERE thread_id = ?'
            await _db_connection.execute(delete_query, (thread_id,))
            deleted_from_tables.append(table_name)

        await _db_connection.commit()
        logger.warning(
            f"Cleared checkpoint state for thread_id={thread_id} from tables: {deleted_from_tables}"
        )
        return True
    except Exception as e:
        logger.error(f"Failed to clear checkpoint state for thread_id={thread_id}: {e}")
        return False
