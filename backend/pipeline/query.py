...
async def run_orchestration_stream(
    ...
    async for chunk in generate_answer_stream(
        ...
        elif chunk["type"] == "spdrag_doc_start":
            yield f"data: {json.dumps({'type': 'spdrag_doc_start', 'document': chunk.get('document', '')})}\n\n"

        elif chunk["type"] == "spdrag_doc_end":
            yield f"data: {json.dumps({'type': 'spdrag_doc_end', 'document': chunk.get('document', '')})}\n\n"

        elif chunk["type"] == "spdrag_synthesis_start":
            yield f"data: {json.dumps({'type': 'spdrag_synthesis_start'})}\n\n"
