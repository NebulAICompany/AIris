...
def retrieve_with_keyword_search(query: str, k: int = 10, selected_files: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    try:
        logger.info(f"🔍 Keyword search for: '{query}' (limit: {k})")
        
        # Perform keyword search
        results = keyword_search(query, k=k, selected_files=selected_files)
...
def retrieve_with_keyword_helping(
    ...
    try:
        ...
        results = keyword_search(query, k=3, selected_files=selected_files)
...
def retrieve_hybrid(
    ...
    try:
        ...
        keyword_results = retrieve_with_keyword_search(query, k=k*2, selected_files=selected_files)
        ...
        if keyword_results:
            ...
            for result in keyword_results:
                ...
                if doc_key in combined_results:
                    ...
                    combined_results[doc_key]['score'] += normalized_score * keyword_weight
                    combined_results[doc_key]['keyword_score'] = result['score']
                    if 'matched_terms' in result['metadata']:
                        combined_results[doc_key]['metadata']['matched_terms'] = result['metadata']['matched_terms']
                else:
        ...
        logger.info(f"✅ Hybrid search: {len(vector_results)} vector + {len(keyword_results)} keyword → {len(final_results)} final results")
