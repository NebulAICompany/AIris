...
try:
    ...
    if not jpype.isJVMStarted():
        ...
        zemberek_path = os.path.join(os.path.dirname(__file__), '..', 'libs', 'zemberek-full.jar')
...
# Niyet tipleri
IntentType = Literal["özet", "trend", "analiz", "karşılaştırma", "tahmin", "soru", "bilinmeyen"]
...
def clean_query(query: str) -> str:
    ...
    # URL'leri temizle
    query = re.sub(r'https?://\S+|www\.\S+|\.com|\.org|\.net|\.edu|\.gov|\.io|\.co|\.ai', '', query)
...
def detect_intent(query: str) -> Tuple[IntentType, float]:
    ...
    intent_keywords = {
        "özet": ["özet", "özetle", "özetler", "özetlemek", "özeti", "özetini", "özetleyebilir", "kısaca", "özetçe"],
        "trend": ["trend", "eğilim", "yönelim", "artış", "azalış", "grafik", "gidişat", "zaman serisi", "değişim"],
        "analiz": ["analiz", "analizi", "değerlendirme", "inceleme", "çözümleme", "neden", "ilişki", "detay", "detaylı"],
        "karşılaştırma": ["karşılaştır", "kıyasla", "karşılaştırma", "fark", "benzerlik", "arasındaki", "kıyas"],
        "tahmin": ["tahmin", "öngörü", "beklenti", "gelecek", "olacak", "olur mu", "olabilir"],
        "soru": ["kim", "ne", "nedir", "kaç", "neden", "nasıl", "nerede", "ne zaman", "hangi"]
    }
