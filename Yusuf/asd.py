...
class VectorStorePipeline:
    ...
    def run(self, uploads_path: str, save_path: str):
        try:
            ...
            # --- PII Masking and Chunk ID Metadata for each chunk ---
            pii_chunk_maps = {}
            for idx, doc in enumerate(docs):
                masked, mapping = pii_mask(doc.page_content)
                doc.page_content = masked
                # Add chunk_id as metadata
                if not hasattr(doc, "metadata") or doc.metadata is None:
                    doc.metadata = {}
                doc.metadata["chunk_id"] = f"chunk_{idx}"
                pii_chunk_maps[f"chunk_{idx}"] = mapping

            # Ensure save_path exists before saving the PII maps
            ...
            # --- Continue with vectorstore logic ---
