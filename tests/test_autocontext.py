...
from backend.retrieval.autocontext import AutoContextProcessor, apply_autocontext
...
async def test_autocontext_processor():
    ...
    processor = AutoContextProcessor(
        use_document_summary=True, use_section_summaries=True
    )
    ...
    try:
        ...
        for i, chunk in enumerate(processed_chunks[:3]):  # Show first 3 chunks
            print(f"\n--- Chunk {i+1} ---")
            print(
                f"Original size: {chunk.metadata.get('original_chunk_size', 'N/A')} chars"
            )
            print(
                f"Contextual size: {chunk.metadata.get('contextual_chunk_size', 'N/A')} chars"
            )
        ...
        # Test metrics
        original_sizes = [
            chunk.metadata.get("original_chunk_size", 0) for chunk in processed_chunks
        ]
        contextual_sizes = [
            chunk.metadata.get("contextual_chunk_size", 0) for chunk in processed_chunks
        ]
        ...
        avg_contextual_size = (
            sum(contextual_sizes) / len(contextual_sizes) if contextual_sizes else 0
        )
        ...
        print(
            f"Average context overhead: {avg_contextual_size - avg_original_size:.1f} characters"
        )
        print(
            f"Context enhancement ratio: {avg_contextual_size / avg_original_size:.2f}x"
        )
...
async def test_apply_autocontext():
    ...
    if enhanced_chunks and unchanged_chunks:
        ...
        print(
            f"First chunk - Enhanced: {enhanced_size} chars, Unchanged: {unchanged_size} chars"
        )
...
async def test_section_extraction():
    ...
    for i, section in enumerate(sections):
        print(
            f"{i+1}. {section['title']} (Level {section['level']}) - {len(section['content'])} chars"
        )
