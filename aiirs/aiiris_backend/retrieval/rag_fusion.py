...
from aiiris_backend.retrieval.retriever import retrieve_top_k
...
async def generate_fusion_queries(original_query: str, num_queries: int = 4) -> List[str]:
    """
...
Generated queries:"""
        ...
        for line in lines:
            line = line.strip()
            if line and (line.startswith(('1.', '2.', '3.', '4.', '5.', '6.', '7.', '8.', '9.')) or line.startswith('-')):
...
def reciprocal_rank_fusion(search_results_dict: Dict[str, List[Dict[str, Any]]], k: int = 60) -> List[Dict[str, Any]]:
    """
    ...
    for query, doc_results in search_results_dict.items():
        ...
        for rank, doc in enumerate(sorted_results):
            ...
            logger.debug(f"Doc {doc_id[:50]}... rank {rank} -> RRF score {rrf_score:.4f}, total: {fused_scores[doc_id]:.4f}")
...
async def retrieve_with_fusion(
    ...
    try:
        ...
        if len(queries) <= 1:
            logger.warning("RAG Fusion: Only original query available, falling back to standard retrieval")
            docs = retrieve_top_k(query, k=k)
        ...
        if not fused_results:
            ...
            return [], {"queries_used": queries, "fusion_applied": True, "error": "No results after fusion"}
        ...
        if final_rerank and len(fused_results) > 1:
            ...
            try:
                ...
                reranked = rerank(query, rerank_docs, with_score=True, top_n=min(top_n * 2, len(rerank_docs)))
                ...
                final_docs = reranked
                logger.info(f"📊 RAG Fusion: Final reranking completed, {len(final_docs)} documents")
        ...
        logger.info(f"✅ RAG Fusion completed: {len(result_docs)} final documents returned")
        ...
        docs = retrieve_top_k(query, k=k)
        return docs[:top_n], {"queries_used": [query], "fusion_applied": False, "error": str(e)} 