...
HALLUCINATION_MARKERS = {
    "aşırı_kesinlik": [
        "kesinlikle", "mutlaka", "asla", "hiç şüphesiz", "her zaman",
        "100% eminim", "net olarak biliyorum", "kategorik olarak",
        "garanti ederim", "şüphesiz"
    ],
    "belirsizlik": [
        "sanırım", "emin değilim", "belki", "tahminimce", "öyle olduğunu düşünüyorum",
        "tam olarak bilemiyorum", "hissetmiyorum", "uğra", "olabilir", 
        "olasılıkla", "muhtemelen", "gibi görünüyor"
    ]
}
...
def check_input_violations(text: str) -> List[str]:
    ...
    for category, words in HARMFUL_KEYWORDS.items():
        matched_words = [word for word in words if re.search(fr'\b{re.escape(word)}\b', lower_text, re.IGNORECASE)]
...
def check_output_violations(text: str) -> List[str]:
    ...
    certainity_markers = sum(1 for marker in HALLUCINATION_MARKERS["aşırı_kesinlik"]
