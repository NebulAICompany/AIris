...
class DocumentVerificationPipeline:
    ...
    def __init__(self):
        ...
        self.openai_client = OpenAI(
            api_key=os.getenv("OPENAI_API_KEY", "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")
        )
        
    def verify_document(self, file_path: str, verification_type: str = "auto", wolfram_enabled: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if verification_type == "auto":
                ...
                results["verification_type"] = classification_result.get("detected_type", "unknown")
            else:
            ...
            consistency_result = self._check_data_consistency(
                ocr_result["extracted_fields"], results["verification_type"], wolfram_enabled
            )
            ...
            results["confidence_score"] = self._calculate_confidence_score(results["stages"])
            ...
            if results["confidence_score"] >= 0.8:
                ...
                results["warnings"].append("Low confidence score - manual review recommended")
            else:
    ...
    def _extract_key_fields(self, text: str) -> Dict[str, Any]:
        ...
        if len(text.strip()) > 200:
            try:
                ...
                logger.warning(f"LLM field extraction failed, using regex only: {str(e)}")
        ...
        try:
            ...
            for key, value in result.items():
                if isinstance(value, list) and value:
                    ...
                    cleaned_value = [str(v).strip() for v in value if v and str(v).strip()]
                    if cleaned_value:
    ...
    def _validate_template_structure(self, file_path: str, text: str, doc_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                penalty = len(missing_fields) * 0.1 + len(format_issues) * 0.05 + \
                         len(calculation_errors) * 0.15 + len(date_issues) * 0.1
                
                final_score = max(0.0, base_score - penalty)
                
                # Compile all issues
                all_issues = missing_fields + format_issues + calculation_errors + date_issues
    ...
    def _check_data_consistency(self, extracted_fields: Dict[str, Any], doc_type: str, wolfram_enabled: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Compile all issues
                all_issues = date_issues + calculation_errors + amount_issues + logical_issues
                
                # Add Wolfram Alpha mathematical verification if enabled
                wolfram_checks = {}
                wolfram_issues = []
                if wolfram_enabled and extracted_fields:
                    wolfram_analysis = self._perform_wolfram_mathematical_verification(extracted_fields, text)
                    ...
                    if wolfram_analysis.get("mathematical_errors_found", 0) > 0:
                        final_score -= 0.2  # Significant penalty for mathematical errors
                    
                    all_issues.extend(wolfram_issues)
                
                return {
                    "consistent": result.get("is_consistent", False) and final_score >= 0.6,
    ...
    def _simple_consistency_check(self, extracted_fields: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _analyze_fraud_risk(self, file_path: str, ocr_result: Dict[str, Any], doc_type: str, wolfram_enabled: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                final_risk_score = min(1.0, base_risk_score + indicator_penalty + pattern_penalty + unrealistic_penalty)
                ...
                all_indicators = fraud_indicators + suspicious_patterns + unrealistic_elements
                
                # Add Wolfram Alpha advanced mathematical fraud detection if enabled
                wolfram_fraud_checks = {}
                if wolfram_enabled:
                    wolfram_fraud_analysis = self._perform_wolfram_fraud_detection(text, ocr_result.get("extracted_fields", {}))
                    ...
                    if wolfram_fraud_analysis.get("mathematical_fraud_score", 0) > 0.3:
                        final_risk_score += 0.3  # Significant increase for mathematical fraud
                ...
                return {
                    ...
                    "wolfram_fraud_analysis": wolfram_fraud_analysis if wolfram_enabled else None
                }
            ...
            return self._simple_fraud_analysis(ocr_result.get("extracted_text", ""), ocr_result)
    
    def _simple_fraud_analysis(self, text: str, ocr_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        suspicious_words = ["photoshop", "fake", "template", "duplicate", "copy", "edited", "modified"]
    ...
    def _perform_wolfram_mathematical_verification(self, extracted_fields: Dict[str, Any], text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not amounts:
                verification_results["issues"].append("No monetary amounts found for Wolfram verification")
            ...
            if len(processed_amounts) >= 2:
                ...
                verification_results.update(self._verify_mathematical_relationships(processed_amounts, text))
            ...
            verification_results.update(self._verify_tax_calculations(text, processed_amounts))
            
            # Set verification flags
            verification_results["checks"]["wolfram_mathematical_verification"] = True
            verification_results["checks"]["calculations_mathematically_correct"] = verification_results["mathematical_errors_found"] == 0
            
            logger.info(f"Wolfram verification completed: {verification_results['calculations_verified']} calculations verified, {verification_results['mathematical_errors_found']} errors found")
    ...
    def _verify_mathematical_relationships(self, amounts: List[float], text: str) -> Dict[str, Any]:
        """Verify mathematical relationships between amounts using Wolfram"""
        results = {"calculations_verified": 0, "mathematical_errors_found": 0, "issues": [], "wolfram_responses": []}
        ...
        try:
            ...
            if len(amounts) >= 3:
                ...
                results["wolfram_responses"].append({"query": query, "response": wolfram_response})
                results["calculations_verified"] += 1
                
                if "true" in wolfram_response.lower() or "yes" in wolfram_response.lower():
                    logger.info("Wolfram confirmed mathematical relationship")
                elif "false" in wolfram_response.lower() or "no" in wolfram_response.lower():
                    results["mathematical_errors_found"] += 1
                    results["issues"].append(f"Mathematical inconsistency: {largest} ≠ {potential_subtotal} + {potential_tax}")
        ...
        try:
            ...
            for percentage in percentages:
                if percentage not in [8, 18, 1, 10]:  # Common Turkish VAT rates
                    ...
                    if "no" in wolfram_response.lower() or "invalid" in wolfram_response.lower():
                        results["issues"].append(f"Unusual VAT rate detected: {percentage}%")
    ...
    def _verify_tax_calculations(self, text: str, amounts: List[float]) -> Dict[str, Any]:
                        ...
                        if base_amount > 0:
                            ...
                            for vat_rate in common_vat_rates:
                                if abs(implied_rate - vat_rate) < 0.001:  # Close match
                                    ...
                                    wolfram_result = self._extract_number_from_wolfram_response(wolfram_response)
                                    if wolfram_result and abs(wolfram_result - total_amount) > 0.01:
                                        results["mathematical_errors_found"] += 1
                                        results["issues"].append(f"VAT calculation error: Expected {wolfram_result}, found {total_amount}")
    ...
    def _perform_wolfram_fraud_detection(self, text: str, extracted_fields: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            fraud_analysis["checks"]["mathematical_fraud_detected"] = fraud_analysis["mathematical_fraud_score"] > 0.3
            
            logger.info(f"Wolfram fraud detection completed with score: {fraud_analysis['mathematical_fraud_score']}")
    ...
    def _detect_impossible_calculations(self, text: str) -> Dict[str, Any]:
        ...
        results = {"indicators": [], "mathematical_fraud_score": 0.0, "wolfram_responses": []}
            ...
            for pattern in impossible_patterns:
                ...
                for match in matches:
                    try:
                        ...
                        elif any(op in text[text.find(match[0]):text.find(match[2])] for op in ['×', '*']):
                            query = f"What is {operand1} * {operand2}?"
                        ...
                        correct_result = self._extract_number_from_wolfram_response(wolfram_response)
                        
                        if correct_result and abs(correct_result - claimed_result) > 0.01:
                            results["indicators"].append(f"Mathematical error in {operation}: {operand1} {'+' if operation == 'addition' else '-' if operation == 'subtraction' else '*' if operation == 'multiplication' else '/'} {operand2} = {claimed_result}, should be {correct_result}")
                            results["mathematical_fraud_score"] += 0.4  # High penalty for calculation errors
            ...
            for pattern in currency_patterns:
                ...
                for match in matches:
                    try:
                        ...
                        converted_amount = self._extract_number_from_wolfram_response(wolfram_response)
                        ...
                        if converted_amount:
                            ...
                            if abs(converted_amount - amount2) > tolerance:
                                results["indicators"].append(f"Suspicious currency conversion: {amount1} {currency1} claimed as {amount2} {currency2}, current rate suggests ~{converted_amount:.2f} {currency2}")
            ...
            for amount in numeric_amounts:
                ...
                if amount > 1000000:  # Over 1 million TL
                    ...
                    if amount > 10000000:  # Over 10 million TL
                        results["indicators"].append(f"Extremely large amount detected: {amount} TL")
